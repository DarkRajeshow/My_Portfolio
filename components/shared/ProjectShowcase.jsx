...
const GridElementFeatured = ({ image, gitHub, title, description, websiteLink, featured = false, technologyUsed, features, impacts }) => {
    const technologies = skills.filter((skill) => technologyUsed.indexOf(skill.name) > -1);
    return (
        <CardContainer className="w-full">
            <CardBody className="bg-texturedBg dark:bg-texturedBgDark relative group/card shadow-[0_0_4px_4px] shadow-zinc-200 dark:shadow-zinc-800 dark:border-light h-auto rounded-2xl p-10 border-2 w-full border-dark my-0">
                <CardItem
                    translateZ="30"
                    as='p'
                    className=' font-semibold text-base text-green-800 dark:text-green-300'>
                ...
                <CardItem
                    translateZ="50"
                    as='p'
                    className=' font-bold text-4xl my-1 text-dark dark:text-light'>
                    {title}
                </CardItem>
                <CardItem
                    as="p"
                    translateZ="30"
                    className="font-medium my-2 dark:text-zinc-200 text-dark/80"
                >
                    {description}
                </CardItem>
                <CardItem
                    ...
                    className="font-medium my-2 dark:text-zinc-200 text-dark/80"
                >
                    <h3 className='text-green-700 font-semibold dark:text-green-300 '>Features</h3>
                    <ul>{features.map((feature, index) => (
                        <li key={index} className='list-none flex gap-1.5 items-center text-sm'>
                            <svg xmlns="http://www.w3.org/2000/svg" className='lucide lucide-circle-check h-3 w-3 text-green-700 dark:text-green-300' width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><path d="m9 12 2 2 4-4" /></svg>
                            {feature}
                        </li>
                    ))}
                    </ul >
                ...
                {impacts && <CardItem
                    ...
                    className="font-medium my-2 dark:text-zinc-200 text-dark/80"
                >
                    <h3 className='text-red-700 font-semibold dark:text-red-300 '>Impacts</h3>
                    <ul>{impacts.map((feature, index) => (
                        <li key={index} className='list-none flex gap-1.5 text-sm'>
                ...
                <div className="flex justify-between items-center mt-10">
                    <CardItem
                        ...
                        href={websiteLink}
                        target="__blank"
                        className="px-4 py-2 font-medium rounded-xl text-sm text-blue-800 dark:text-blue-200"
                    >
                        Visit site →
                    </CardItem>
                    <CardItem
                        ...
                        href={gitHub}
                        target="__blank"
                        className="px-4 py-2 rounded-xl bg-black dark:bg-white dark:text-black text-white text-sm font-semibold flex gap-2 items-center justify-center"
                    >
...
export default function ProjectShowcase() {
    return (
        <>
            <AnimateText className='my-10 md:my-16 lg:my-24 font-extrabold text-center shadow-purple' text="Want to see My work? Here it is." durationtime='1' />

            <div className="title">
                <h1 className='text-2xl font-semibold tracking-widest'>Featured Projects</h1>
                <p className='text-sm my-1 text-gray-400'>These project shows my current level.</p>
            </div>
            <div className="grid grid-cols-2 gap-10 pb-64">
                {projects.slice(0, 6).map((project, index) => (
                    <GridElementFeatured key={index} image={project.image} websiteLink={project.websiteLink} features={project.features} gitHub={project.gitHub} technologyUsed={project.technologyUsed} title={project.title} impacts={project?.impacts} description={project.description} featured={project.featured} />
                ))}
            </div>
            <h1 className='text-2xl font-semibold tracking-widest'>Progress Projects</h1>
            <div className="grid grid-cols-2 gap-10 pb-64">
